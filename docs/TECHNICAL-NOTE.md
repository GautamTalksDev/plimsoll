...
**A single-operator log can still equivocate, but the gap is narrower when the log is git-backed.** An operator who only ever served tree heads over HTTP could present different heads to different verifiers; Merkle consistency proofs make that detectable only among parties that compare the heads they received. Certificate Transparency closes more of that gap with gossip and witness protocols. **No witness or gossip protocol is specified here, and git mirroring is not a witness protocol.**

What git mirroring **does** give you: the public Plimsoll Log is a public git repository. Every clone holds the complete append history. A rewritten history fails to fast-forward for anyone who already held a prior clone (`git fetch` / `git status`). Independently, `plimsoll verify-log --dir` recomputes every Merkle leaf from stored canonical bytes, rebuilds each checkpoint root, and verifies every Ed25519 signature against the published public key (see the accompanying `docs/MIRRORING.md`). Storage is append-only by two mechanisms: git history and the SQLite triggers.

What it **does not** give you: automatic detection if two parties never compare (neither clones nor shared heads); a CT-style witness quorum; or binding of wall-clock timestamps to an external time authority. The practical mitigations that remain are unchanged: any party may operate their own conforming log, and verification is not tied to any particular endpoint.
